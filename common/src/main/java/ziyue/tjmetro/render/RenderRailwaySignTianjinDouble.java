...
public class RenderRailwaySignTianjinDouble<T extends BlockRailwaySignTianjinDouble.TileEntityRailwaySignTianjinDouble> extends BlockEntityRendererMapper<T> implements IBlock, IGui, IDrawing
{
    public RenderRailwaySignTianjinDouble(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    }
    ...
    public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
        ...
        if (!(state.getBlock() instanceof BlockRailwaySignTianjinDouble block)) return;
        ...
        storedMatrixTransformations.add(matricesNew -> {
            ...
            matricesNew.translate(block.getXStart() / 16F - 0.5, -0.5, -0.0625 - SMALL_OFFSET * 2);
        });
    ...
    public static void renderCustomText(String signText, StoredMatrixTransformations storedMatrixTransformations, Direction facing, float size, float start, float offset, boolean flipCustomText, float maxWidth, int backgroundColor) {
        final ClientCache.DynamicResource dynamicResource = ClientCache.DATA_CACHE.getSignText(signText, flipCustomText ? IGui.HorizontalAlignment.RIGHT : IGui.HorizontalAlignment.LEFT, (1 - BlockRailwaySign.SMALL_SIGN_PERCENTAGE) / 2, backgroundColor, ARGB_WHITE);
