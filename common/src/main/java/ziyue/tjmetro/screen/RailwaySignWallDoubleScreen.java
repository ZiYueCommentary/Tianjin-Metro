...
public class RailwaySignWallDoubleScreen extends ScreenMapper implements IGui
{
    ...
    public RailwaySignWallDoubleScreen(BlockPos signPos) {
        ...
        if (world != null) {
            ...
            if (entity instanceof BlockRailwaySignWallDouble.TileEntityRailwaySignWallDouble entityRailwaySign) {
    ...
    protected void init() {
        ...
        if (!isRailwaySign && minecraft != null) {
            UtilitiesClient.setScreen(minecraft, new DashboardListSelectorScreen(this::onClose, platformsForList, selectedIds.get(line), true, false));
        }
    }
            ...
            if (editingIndex >= 0) {
                ...
                loopSigns((index, x, y, isBig) -> {
                    ...
                    if (sign != null) {
                        ...
                        RenderRailwaySign.drawSign(matrices, null, null, font, signPos, signId, (isBig ? xOffsetBig : xOffsetSmall) + x + (moveRight ? SIGN_BUTTON_SIZE * 2 : 0), BUTTON_Y_START + y, SIGN_BUTTON_SIZE, 2, 2, selectedIds.get(line), Direction.UP, 0, (textureId, x1, y1, size, flipTexture) -> blit(matrices, (int) x1, (int) y1, 0, 0, (int) size, (int) size, (int) (flipTexture ? -size : size), (int) size));
                    }
                }, false);
    ...
    @Override
    public void onClose() {
        PacketGuiServer.sendSignIdsDoubleC2S(signPos, selectedIds, signIds);
        super.onClose();
    }
