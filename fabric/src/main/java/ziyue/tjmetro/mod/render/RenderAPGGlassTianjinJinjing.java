...
import ziyue.tjmetro.mod.block.BlockAPGGlassTianjinJinjing;
 ...
 * @see RenderAPGGlassTianjinJinjing
 * @see RenderRouteBase
 * @since 1.0.0-prerelease-1
 */

public class RenderAPGGlassTianjinJinjing extends RenderRouteBase<BlockAPGGlassTianjinJinjing.BlockEntity>
{
    public static final float COLOR_STRIP_START = 15.5F / 16;
    public static final float COLOR_STRIP_END = 1F;

    public RenderAPGGlassTianjinJinjing(Argument dispatcher) {
        super(dispatcher, 4, 8F, 0.5F, 8, false, 2, BlockAPGGlass.ARROW_DIRECTION);
    }
    ...
    @Override
    public void render(BlockAPGGlassTianjinJinjing.BlockEntity entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        InitClient.findClosePlatform(blockPos.down(platformSearchYOffset), 5, platform -> {
            ...
            if ((renderType != RenderType.NONE) && IBlock.getStatePropertySafe(state, SIDE_EXTENDED) != EnumSide.SINGLE) {
                ...
                final Identifier identifier = DynamicTextureCache.instance.getRouteMapJinjing(platformId, arrowDirection, width / height, transparentWhite).identifier;
